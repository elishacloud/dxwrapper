...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateVertexShader(CONST DWORD *pDeclaration, CONST DWORD *pFunction, DWORD *pHandle, DWORD Usage)
{
	...
	*pHandle = 0;
	...
	while (i < limit)
	{
		...
		{
			// LOG << "> Failed because token type '" << token_type << "' is not supported!" << std::endl;
	...
	if (pFunction != nullptr)
	{
		...
		if (FAILED(hr))
		{
			if (errors != nullptr)
			{
				// LOG << "> Failed to reassemble shader:" << std::endl << std::endl << static_cast<const char *>(errors->GetBufferPointer()) << std::endl;
