...
void WINAPI D3d9Wrapper::genericQueryInterface(REFIID riid, LPVOID *ppvObj, m_IDirect3DDevice9Ex* m_pDeviceEx)
{
	...
	{
		*ppvObj = m_pDeviceEx->ProxyAddressLookupTable->FindAddress<m_IDirect3DSwapChain9Ex, m_IDirect3DDevice9Ex>(*ppvObj, m_pDeviceEx, riid);
		return;
	}
...
#define QUERYINTERFACE(x) \
	if (riid == IID_ ## x) \
		{ \
			*ppvObj = m_pDeviceEx->ProxyAddressLookupTable->FindAddress<m_ ## x,m_IDirect3DDevice9Ex>(*ppvObj, m_pDeviceEx, riid); \
			return; \
		}
