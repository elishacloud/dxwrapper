...
{
	LOG << "UnhandledExceptionFilter: exception code=" << ExceptionInfo->ExceptionRecord->ExceptionCode <<
	...
	case 0xc0000005: // Memory exception (Tie Fighter)
		...
		LOG << "UnhandledExceptionFilter: NOP opcode=" << std::showbase << std::hex << *(BYTE *)target << std::dec << std::noshowbase << " len=" << cmdlen;
		...
		{
			LOG << "UnhandledExceptionFilter: FlushInstructionCache ERROR target=" << std::showbase << std::hex << target << std::dec << std::noshowbase << ", err=" << GetLastError();
		}
...
{
#ifdef _DEBUG
	LOG << "SetUnhandledExceptionFilter: lpExceptionFilter=" << lpTopLevelExceptionFilter;
#endif
...
static HMODULE LoadDisasm()
{
	...
	if (!disasmlib)
	{
		LOG << "Load lib=" << buffer << " failed err=" << GetLastError();
		return nullptr;
	}
...
#ifdef _DEBUG
	LOG << "Load " << buffer << " ptrs=" << pGeterrwarnmessage << std::showbase << std::hex << pPreparedisasm << pFinishdisasm << pDisasm << std::dec << std::noshowbase;
#endif
	return disasmlib;
}
