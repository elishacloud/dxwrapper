...
HRESULT m_IDirectDrawX::SetDisplayMode(DWORD dwWidth, DWORD dwHeight, DWORD dwBPP, DWORD dwRefreshRate, DWORD dwFlags)
{
	...
	if (Config.Dd7to9)
	{
		...
		if (IsExclusiveMode()
			&& LastWidth && LastHeight && Device.Width && Device.Height && LastWidth != Device.Width && LastHeight != Device.Height
			&& LastBPP && DisplayMode.BPP && LastBPP != DisplayMode.BPP)
		{
...
void m_IDirectDrawX::ReleaseInterface()
{
	...
	ProxyAddressLookupTableDdraw.SaveInterfaceAddress(WrapperInterface4);
...
void* m_IDirectDrawX::GetWrapperInterfaceX(DWORD DirectXVersion)
{
	...
	case 0:
		...
		break;
	case 1:
		return ProxyAddressLookupTableDdraw.GetInterfaceAddress(WrapperInterface, (LPDIRECTDRAW)ProxyInterface, this);
	case 2:
		return ProxyAddressLookupTableDdraw.GetInterfaceAddress(WrapperInterface2, (LPDIRECTDRAW2)ProxyInterface, this);
	case 3:
		return ProxyAddressLookupTableDdraw.GetInterfaceAddress(WrapperInterface3, (LPDIRECTDRAW3)ProxyInterface, this);
	case 4:
		return ProxyAddressLookupTableDdraw.GetInterfaceAddress(WrapperInterface4, (LPDIRECTDRAW4)ProxyInterface, this);
	case 7:
		return ProxyAddressLookupTableDdraw.GetInterfaceAddress(WrapperInterface7, (LPDIRECTDRAW7)ProxyInterface, this);
	}
	LOG_LIMIT(100, __FUNCTION__ << " Error: wrapper interface version not found: " << DirectXVersion);
	return nullptr;
}
...
HRESULT m_IDirectDrawX::CreateD9Device(char* FunctionName)
{
	...
	// Set behavior flags
	BehaviorFlags = ((D9Cache.Caps9.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT) ? D3DCREATE_MIXED_VERTEXPROCESSING : D3DCREATE_SOFTWARE_VERTEXPROCESSING) |
	...
	struct SetWindowFullScreen {
		static void FullScreen(HMONITOR hMon, HWND hWnd, DWORD Width, DWORD Height)
		{
			if (Config.FullscreenWindowMode || !Config.EnableWindowMode)
			{
				...
				m_IDirect3D9Ex::AdjustWindowSize(hMon, hWnd, Width, Height, false, true);
			...
			{
				m_IDirect3D9Ex::AdjustWindowSize(hMon, hWnd, Width, Height, true, false);
			}
		}
	};
...
{
	// If incorrect param or incorrect device
	if (!CreationInterface || hWnd != presParams.hDeviceWindow || !IsWindow(DisplayMode.hWnd))
	{
		return;
	}
	...
	for (const auto& pDDraw : DDrawVector)
	{
		if (pDDraw == CreationInterface)
		{
			pDDraw->ReleaseAllD9Resources(false, false);
			pDDraw->ReleaseD9Device();
		}
	}
}
