...
#include <DDrawCompat/v0.3.1/Direct3d/Direct3d.h>
#include <DDrawCompat/v0.3.1/Direct3d/Direct3dDevice.h>
#include <DDrawCompat/v0.3.1/Direct3d/Direct3dExecuteBuffer.h>
#include <DDrawCompat/v0.3.1/Direct3d/Direct3dLight.h>
#include <DDrawCompat/v0.3.1/Direct3d/Direct3dMaterial.h>
#include <DDrawCompat/v0.3.1/Direct3d/Direct3dTexture.h>
#include <DDrawCompat/v0.3.1/Direct3d/Direct3dVertexBuffer.h>
#include <DDrawCompat/v0.3.1/Direct3d/Direct3dViewport.h>
...
namespace
{
	...
	CompatPtr<TDirect3d> createDirect3d(CompatRef<TDirectDraw> dd)
	{
		...
		HRESULT result = dd->QueryInterface(&dd, Compat31::getIntfId<TDirect3d>(),
		...
		{
			Compat31::Log() << "ERROR: Failed to create a Direct3D object for hooking: " << Compat31::hex(result);
		}
		return d3d;
	}
	...
	CompatPtr<IDirectDrawSurface7> createRenderTarget(CompatRef<IDirectDraw7> dd)
	{
		...
		if (FAILED(result))
		{
			Compat31::Log() << "ERROR: Failed to create a render target for hooking: " << Compat31::hex(result);
		}
		return renderTarget;
	}
	...
	void hookDirect3dDevice(CompatRef<IDirect3D3> d3d, CompatRef<IDirectDrawSurface4> renderTarget)
	{
		...
		if (FAILED(result))
		{
			Compat31::Log() << "ERROR: Failed to create a Direct3D device for hooking: " << Compat31::hex(result);
			return;
		}
	...
	void hookDirect3dExecuteBuffer(CompatRef<IDirect3DDevice> dev)
	{
		...
		if (FAILED(result))
		{
			Compat31::Log() << "ERROR: Failed to create an execute buffer for hooking: " << Compat31::hex(result);
			return;
		}
	...
	void hookDirect3dLight(CompatRef<IDirect3D3> d3d)
	{
		...
		if (FAILED(result))
		{
			Compat31::Log() << "ERROR: Failed to create a light for hooking: " << Compat31::hex(result);
			return;
		}
	...
	void hookDirect3dMaterial(CompatRef<IDirect3D3> d3d)
	{
		...
		if (FAILED(result))
		{
			Compat31::Log() << "ERROR: Failed to create a material for hooking: " << Compat31::hex(result);
			return;
		}
	...
	void hookDirect3dTexture(CompatRef<IDirectDraw> dd)
	{
		...
		if (FAILED(result))
		{
			Compat31::Log() << "ERROR: Failed to create a texture for hooking: " << Compat31::hex(result);
			return;
		}
	...
	void hookDirect3dVertexBuffer(CompatRef<IDirect3D3> d3d)
	{
		...
		if (FAILED(result))
		{
			Compat31::Log() << "ERROR: Failed to create a vertex buffer for hooking: " << Compat31::hex(result);
		}
	...
	void hookDirect3dViewport(CompatRef<IDirect3D3> d3d)
	{
		...
		if (FAILED(result))
		{
			Compat31::Log() << "ERROR: Failed to create a Direct3D viewport for hooking: " << Compat31::hex(result);
			return;
		}
