...
namespace
{
	...
	Compat30::SrwLock g_windowProcSrwLock;
	...
	{
		LOG_FUNC("ddcWindowProc", Compat30::WindowMessageStruct(hwnd, uMsg, wParam, lParam));
		...
		decltype(&GetWindowLongA) origGetWindowLong, WNDPROC(WindowProc::* wndProc))
	{
		...
		{
			Compat30::ScopedSrwLockExclusive lock(g_windowProcSrwLock);
	...
	bool isUser32ScrollBar(HWND hwnd)
	{
		...
		return GetModuleHandle("comctl32") != Compat30::getModuleHandleFromAddress(
	...
	{
		LOG_FUNC("SetWindowPos", hWnd, hWndInsertAfter, X, Y, cx, cy, Compat30::hex(uFlags));
