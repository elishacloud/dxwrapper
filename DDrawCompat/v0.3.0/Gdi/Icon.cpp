...
namespace
{
	...
	Result WINAPI iconFunc(Params... params)
	{
		...
		{
			return LOG_RESULT(Compat30::g_origFuncPtr<origFunc>(params...));
		}

		Gdi::DcFunctions::disableDibRedirection(true);
		Result result = Compat30::g_origFuncPtr<origFunc>(params...);
	...
	void hookIconFunc(const char* moduleName, const char* funcName)
	{
		...
		Compat30::hookFunction<origFunc>(moduleName, funcName, &iconFunc<origFunc>);
	}
