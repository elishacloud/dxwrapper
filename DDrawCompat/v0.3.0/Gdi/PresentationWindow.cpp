...
namespace
{
	...
	{
		LOG_FUNC("messageWindowProc", Compat30::WindowMessageStruct(hwnd, uMsg, wParam, lParam));
		...
		case WM_CREATEPRESENTATIONWINDOW:
		{
			...
			static auto origCreateWindowExA = reinterpret_cast<decltype(&CreateWindowExA)>(
				Compat30::getProcAddress(GetModuleHandle("user32"), "CreateWindowExA"));
	...
	{
		LOG_FUNC("presentationWindowProc", Compat30::WindowMessageStruct(hwnd, uMsg, wParam, lParam));
	...
	unsigned WINAPI presentationWindowThreadProc(LPVOID /*lpParameter*/)
	{
		...
		if (!g_messageWindow)
		{
			Compat30::Log() << "ERROR: Failed to create a message-only window";
			return 0;
		}
