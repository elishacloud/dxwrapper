...
namespace
{
	...
	unsigned getInstructionSize(void* instruction)
	{
		...
		if (FAILED(result))
		{
			LOG_ONCE("ERROR: DbgEng: WriteVirtual failed: " << Compat30::hex(result));
			return 0;
		}
		...
		if (FAILED(result))
		{
			LOG_ONCE("ERROR: DbgEng: Disassemble failed: " << Compat30::hex(result) << " "
				<< Compat30::hexDump(instruction, MAX_INSTRUCTION_SIZE));
			return 0;
		}
	...
	void hookFunction(void*& origFuncPtr, void* newFuncPtr, const char* funcName)
	{
...
#ifdef DEBUGLOGS
		oss << Compat30::funcPtrToStr(targetFunc) << ' ';
...
#ifdef DEBUGLOGS
			oss << Compat30::hexDump(prevTargetFunc, instructionSize) << " -> " << Compat30::funcPtrToStr(targetFunc) << ' ';
			prevTargetFunc = targetFunc;
#endif
		}

		if (Compat30::getModuleHandleFromAddress(targetFunc) == Dll::g_currentModule)
		{
			Compat30::Log() << "ERROR: Target function is already hooked: " << funcName;
			return;
		}
		...
		LOG_DEBUG << "Hooking function: " << funcName
			<< " (" << oss.str() << Compat30::hexDump(targetFunc, totalInstructionSize) << ')';
	...
	bool initDbgEng()
	{
		...
		{
			Compat30::Log() << "ERROR: DbgEng: object creation failed: " << Compat30::hex(result);
			return false;
		}
		...
		if (FAILED(result))
		{
			Compat30::Log() << "ERROR: DbgEng: WaitForEvent failed: " << Compat30::hex(result);
			return false;
		}
		...
		if (FAILED(result))
		{
			Compat30::Log() << "ERROR: DbgEng: GetModuleParameters failed: " << Compat30::hex(result);
			return false;
		}
		...
		{
			Compat30::Log() << "ERROR: DbgEng: GetValidRegionVirtual failed: " << Compat30::hex(result);
			return false;
		}
